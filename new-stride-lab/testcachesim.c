...
/*                    testcachesim                                */
/*                                                                */
/*    Author: Noah Mendelsohn (edited by Richard Townsend)        */
/*                                                                */
...
int strided_accesses(int element_count, int stride);
...
void do_stuff(ELEMENT_TYPE *data, struct parsed_args *parsed_argsp)
{
  ...
  for (j = 0; j < VISIT_COUNT; j++) {
    for (i = 0; i < parsed_argsp->element_count; i += parsed_argsp->stride) {
      /*
                                ...
                                int stride, bool cache_sum)
{
  ...
  if (element_count > 1) {
    printf("Elements in the array are accessed with "
           "write/read pairs.\n");
  ...
  if (cache_sum) {
    printf("A running sum of array elements was maintained"
           "and simulated as cacheable.\n\n");
  } else {
    printf("No sum of array elements was maintained. All simulated "
...
void usage()
{
  fprintf(stderr, "Usage: %s  [ -assoc nnn ]  [ -stride nnn ]  [-cachesum]"
                  "-lines nnn -blksize nnn   -elements nnnn \n", progname);
  exit(EXIT_FAILURE);
}
